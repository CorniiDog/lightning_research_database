        ...
        for filt in filters:
            if isinstance(filt, tuple) and len(filt) == 3:
                ...
                if col is None or val is None:
                    raise ValueError("Each filter dict must have 'column' and 'value' keys.")
                conditions.append(f"{col} {op} ?")
                params.append(val)
            else:
                raise ValueError("Filters must be tuples (column, operator, value) or dicts.")
    else:
...
def _parse_dat_extension(lylout_path: str, DB_PATH: str = "lylout_db.db"):
    ...
    for line in lines[data_start_index:]:
        ...
        event = (time_unix, lat, lon, alt, reduced_chi2, num_stations,
                 power_db, power, mask_str, stations_list, x, y, z)
